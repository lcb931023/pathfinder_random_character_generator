angular.module('diceParser', [])
	.factory('diceParser', function(){
		var roll = function (notation) {
			var regex = /(\d+)d(\d*)[\*x]?(\d+)?\+?(\d+)?/;
			var splitted = notation.match(regex);
			// first item in res array is the full match;
			//  the rest are the parenthesized elements
			// if an element doesn't exist,
			//  it'll be a null element in the array
		...
		var roll_max = function (notation) {
			var regex = /(\d+)d(\d*)[\*x]?(\d+)?\+?(\d+)?/;
			var splitted = notation.match(regex);
			// first item in res array is the full match;
			//  the rest are the parenthesized elements
